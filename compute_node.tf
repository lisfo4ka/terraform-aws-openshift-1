...
resource "aws_autoscaling_group" "compute_node" {
  ...
  vpc_zone_identifier = [
    "${coalescelist(
      slice(var.private_subnet_ids, 0, lookup(var.compute_nodes[count.index], "node_count") <
        length(var.private_subnet_ids) ? lookup(var.compute_nodes[count.index], "node_count") :
        length(var.private_subnet_ids)
      ),
      list(var.private_subnet_ids[0])
    )}",
  ]
