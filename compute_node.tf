resource "aws_launch_configuration" "compute_node" {
  ...
  iam_instance_profile = "${var.create_iam_profiles ? coalesce(aws_iam_instance_profile.slave_node.name, var.slave_node_iam_profile_name) : var.slave_node_iam_profile_name}"
