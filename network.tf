...
locals {
  ...
  infra_scaling_subnet_ids = "${split(",", var.infra_node_count < length(var.private_subnet_ids) ? join(",", slice(var.private_subnet_ids, 0, var.infra_node_count)) : join(",", var.private_subnet_ids))}"
}
